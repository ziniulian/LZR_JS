...
- 设计反思：
	...
	- 对使用 this.constructor 的方法的优化，特别在枚举中可能用于直接用类调用的方法。（无果）
	- 通过值匹配枚举值
	- 污染等级 加入健康提示
	- 封装气象枚举
	- 封装风力枚举
...
##### 2016-9-20（ 修正Node.服务ip无法赋值的BUG ）：
	Node.服务 ： 
		修改 属性 ip 的默认值为 null

##### 2016-9-19（ nodejs 使用 express 框架 ）：
	Node.服务 ： 新增类
	Node.路由器 ： 新增类
	Node.工具包 ： 新增类
	LZR：
...
##### 2016-9-13（ 修正遮罩状态下图例无法切换的BUG ）：
	HTML.小部件.图例.图例管理器 ：
		修改 计算中部遮罩数据 方法 ：
			新增 遮罩的最小值和宽度 参数，以解决图例切换时无法获取遮罩的样式数据问题。

##### 2016-9-7（ 日刻度时间控制器添加周末样式 ）：
	HTML.基础.Doe控制器.日刻度时间控制器.日刻度时间控制器信息 ： 
		修改 填充日模块方法 方法 ： 添加周末样式

##### 2016-9-6（ 修正图例遮罩后输出数值与颜色不匹配问题 ）：
	HTML.小部件.图例.图例管理器 ：
		修改 中部开关 属性名 及 构造参数
		修改 构造器 方法 ： 初始化中部开关
		修改 计算中部遮罩数据 方法 ：
		...
		调用计算中部遮罩数据方法的参数修正
	HTML.小部件.图例.色块图例 ：
		修改 生成中部遮罩背景 方法 ： 规范语法，防止IE下样式错误。
	HTML.小部件.图例.渐变色图例 ： 
		修改 生成中部遮罩背景 方法 ： 规范语法，防止IE下样式错误。

##### 2016-9-1（ 日刻度时间控制器添加日均切换功能 ）：
	HTML.基础.Doe控制器.日刻度时间控制器 ： 
		修改 按控制器 属性 ： 取消双击功能
		修改 构造器 方法 ： 添加滚动条鼠标经过提示
		新增 处理经过提示事件 方法
		新增 隐藏经过提示 方法
		修改 添加 & 移除元素事件集 方法 ： 关联鼠标经过事件 和 日均切换事件
	HTML.基础.Doe控制器.日刻度时间控制器.日刻度时间控制器信息 ： 
		添加 是否循环播放 属性
		添加 播放间距 属性
		修改 设置时间范围 方法 ： 修正计算模块数的算法
		修改 播放 方法 ： 统一使用 下一帧 方法控制播放
		修改 下一帧 方法 ： 兼容循环播放和播放间距
		修改 上一帧 方法 ： 兼容循环播放和播放间距
		修改 处理小时变化 方法 ： 保证滚动条的值不会超出时间范围
		新增 处理播放间距变化 方法

##### 2016-8-30（ 新增日刻度时间控制器 ）：
	HTML.基础.Doe控制器.日刻度时间控制器 ： 新增类
	HTML.基础.Doe控制器.日刻度时间控制器.日刻度时间控制器信息 ： 新增类

##### 2016-8-24（ 缩略图添加滚动系数 ）：
	HTML.基础.Doe控制器.缩略图控制器.缩略图信息 ： 
		添加 滚动系数 属性
		提取出 计算min位置 的方法
	HTML.基础.Doe控制器.缩略图控制器 ： 
		修改 处理底图滚动 方法 ： 考虑滚动系数

##### 2016-8-16（ 修改缩略图 ）：
	HTML.基础.Doe控制器.缩略图控制器.缩略图信息 ： 
		修改 重置图片显示个数 方法 ： 大小变化时的布局调整
		修改 处理选中值变化 方法 ： 修正回调的图片参数

##### 2016-8-15（ 元素添加创建事件方法 ）：
	HTML.基础.元素 ：
		添加 创建事件 方法
	HTML.基础.Doe控制器.缩略图控制器 ： 
		修改 给元素添加事件集 方法 ： 添加 resize 事件
		修改 移除元素的事件集 方法 ： 删除 resize 事件
	HTML.基础.Doe控制器.缩略图控制器.缩略图信息 ： 
		修改 重置图片大小 方法 ： 修正滑块被替换 BUG
		修改 重置图片显示个数 方法 ： 可设置固定大小为各种单位

##### 2016-8-14（ 新增缩略图 ）：
	后处理修改 ： 优化类方法绑定的所属类信息解析，防止被翻译成静态属性。
	HTML.基础.Doe控制器.鼠标控制器 ：
		修改 抬起事件 方法 ： 添加鼠标键参数
	HTML.基础.Doe控制器.缩略图控制器 ： 新增类
	HTML.基础.Doe控制器.缩略图控制器.缩略图信息 ： 新增类

##### 2016-8-1（ 完善枚举 ）：
	基础.值.枚举 ： 
		添加 通过枚举值获取枚举 方法
	项目.环保.空气质量.污染物.污染物级别 ： 
		添加 健康提示 属性
...
##### 2016-7-28（ 处理中部遮罩时的拾色 ）：
	HTML.小部件.图例 ： 
		修改 显示拾色器 方法 ： 处理中部遮罩时的拾色
