...
- 设计反思：
	...
	- nodejs 版的 ajax 工具
	- mongodb 查询语句的参数优化
	- mongodb 加入 ObjectID 类
	- 修正 mongodb 只能以对象作查询参数，不能以数组作查询参数的问题。
	- Json类 兼容序列化方法
	- 数据类添加序列化方法
	- 数据类的结构输出增加属性参数
	- 设计数据类数据库（root + id + items，每个item保存id与root结构的路径）
	- 数据序列化可不显示 cls
	- 通用的数据服务，暂不支持post方式传输大数据
	- 修改获取URL中get参数的方法
	- NodeAjax 添加转码参数
	- Node.服务.数据管理器服务，将所有 \_id 改为 id
	- Node.服务.数据管理器服务，修正 删除数据的回调处理 方法 根目录元素删除失败的BUG
	- 股票获取基本面信息
	- 日记插入参数改用年月日格式（srvSetBlog）
	- 网页追踪时记录IP
	- 统一服务返回的数据结构
	- 调试 Openshift V3 （原始TCP通信无法测试成功）
...
- 拆分 openshift 服务：
	...
	- 时间服务
	- 股服务
		- 股票分析，基本面查询与分析，交易统计，根据交易记录自动换算均价及操作参数
	- 访问统计服务
		- 网站访问统计页面
	- 日记服务
		- 重新整理相亲记结构
...
- 设计反思：
	...
	- 基础的mongodb数据库查询、导入、导出功能
	- 研究 Openshift3 的路由、思考如何实现原始TCP通信
	- 域名解析不支持 https 问题
	...
	- 优化Mongo数据库的参数解析，尝试参数的递归替换和键名替换。思考这样鸡肋的改变对性能到底会带来多大的影响，使用上到底又有多大的改变？
...
##### 2017-10-13 （ 修正端口的数据类型 ）：
	Node.服务 ：
		修改 端口 属性 ： 数据类型改为 int

##### 2017-9-21 （ 统一服务返回的数据结构 ）：
	新增 Node.服务.返回结果 ： 规范返回值的数据结构
	Node.服务.数据管理器服务 : 修改所有返回结果的数据结构
		新增 结果类 属性
		新增 返回数据集 方法
...
##### 2017-9-14 （ 夏令时时差临时解决方法 ）：
	基础.时间工具 :
		换算日时间戳时存在夏令时时差问题，现暂时忽略夏令时，来测试程序是否会正常。若仍然不行，计划采用UTC时间进行换算。

##### 2017-8-30 （ 数据服务改用id ）：
	Node.服务.数据管理器服务 :
		将所有 \_id 改为 id
		修正 删除数据的回调处理 方法 : 解决根目录元素删除失败的问题
	HTML.工具.URL工具 :
		修改 获取URL参数 方法 : 解决乱码问题
	Node.数据库.Ajax :
		新增 字符转换器 属性
		新增 字符编码 属性
		修改 执行查询 方法 : 支持字符转换功能
...
##### 2017-7-26 （ 数据服务 ）：
	...
	基础.数据：
		...
		修改 生成序列化对象 方法
		修改 序列化 方法 ： 可不显示 cls
		修改 序列化格式处理 方法 ： 可不显示 cls
	Node.数据库：
		修改 对构造参数的特殊处理 方法
		修改 生成查询语句的事件 方法 ： 改为在此方法中对 查询语句 属性进行修改。
	Node.数据库.Mongo：
		修改 参数解析 方法 ： 尝试对参数进行递归替换和对键名的替换，但未成功，仍有问题。
	删除 Node.服务接口：该接口没啥作用
	新增 Node.服务.数据管理器服务： 专门负责 “基础.数据”型 数据的增删改查

##### 2017-6-20 （ 数据序列化 ）：
	LZR：
		修改 加载js 方法 ： 不再区分 nodejs 及 网页，统一使用 eval 方法
		修改 加载外部库 方法 ： 对 JSON 是否加载进行特殊处理
	基础.数据：
		新增 Json工具 属性
		修改 递归创建子数组 方法 ： cls_ 参数可以使用字符串
		修改 结构输出 方法 ： 新增显示属性参数
		新增 生成序列化对象 方法 ： 可对数据进行序列化
		新增 序列化格式处理 方法 ： 序列化对象转 JSON 前的预处理
		新增 序列化 方法 ： 生成数据的 JSON 格式
		修改 克隆 方法 ： 统一使用序列化对象进行克隆
	基础.JSON：
		修改 将对象转换成 json 文本 方法 ： 兼容数据的序列化方法

##### 2017-6-7 （ Mongo数据库 可匹配数组型参数 ）：
	Node.数据库.Mongo：
		修改 参数解析 方法 ： 匹配数组型参数

##### 2017-5-27 （ 更改 Mongo数据库 参数传递的方式 ）：
	Node.数据库.Mongo：
		新增 ID类 属性
		新增 参数解析 方法

##### 2017-5-27 （ 数据库通用类、nodejs的Ajax类 ）：
	基础.字符串：
		新增 将二进制数据转换为Base64字串 方法
		新增 解析Base64数据为二进制数据 方法
	新增 Mongo数据库通用DAO 类
	新增 nodejs 版的 ajax 工具 类
...
##### 2017-3-27 （ 不能嵌套调用主类的闭包方法 ）：
	工具包 ：
		修改 闭包调用 方法

##### 2017-3-15 （ 闭包调用方法移入主类 ）：
	LZR ： 将工具包的 闭包调用 移入 LZR 主类
		新增 闭包调用 方法
	工具包 ：
		修改 闭包调用 方法

##### 2017-3-8 （ 温度标签管理系统数据库功能 ）：
	Node.路由器 ：
		重命名 路由 delete 匹配 方法
		修改 设置静态文件夹 方法
	项目.温度标签管理系统 ：添加与 MySql 数据库交互功能

##### 2017-1-25 （ 新增温度标签管理系统 ）：
	项目.温度标签管理系统 ：新增类
